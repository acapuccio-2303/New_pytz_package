...
# UnivariateTransformerMixin non c'e' piu', si usa TransformerMixin
...
class RecurrencePlot(BaseEstimator, UnivariateTransformerMixin):  # noqa: D207
    ...
    def transform(self, X, dist_name):
        ...
        dist_name: name of the type of distance to use.
        
        k = used to calculate the corresponding distance.
        ...
        if dimension == 1:
            # La distanza x1, x2:  |x1 – x2| = |Δx| usualmente considerata sulla retta numerica [] 
            # può essere considerata un'evidente estensione della distanza urbanistica al caso monodimensionale; 
            # ma è anche un'estensione della distanza euclidea, infatti: √((Δx)2) = |Δx|.
            X_dist = np.abs(X[:, :, None] - X[:, None, :])
        else:
            
            # scelta parametro k in base alla scelta della distanza
            dist_name = dist_name.casefold()
            if dist_name == 'manhattan' or dist_name == 'geometria del taxi':
                k = 1
            elif dist_name == 'euclidea': 
                k = 2
            elif dist_name == 'chebysev':
                k = inf
            else: 
                print("Distance name not valid, euclidean distance will be used.")
                k = 2
                
            X_traj = _trajectories(X, dimension, time_delay)
            # formula modificata per renderla generica con il parametro k
            X_dist = np.power(
                np.sum((X_traj[:, None, :, :] - X_traj[:, :, None, :]) ** k,
                       axis=3), 
                1/k
            )
